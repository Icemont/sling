    ...
    <x-slot name="header">
        <div class="d-flex">
            <ol class="page-title breadcrumb breadcrumb-arrows" aria-label="breadcrumbs">
                ...
                <li class="breadcrumb-item active" aria-current="page">{{ __('Show #:id', ['id' => $paymentMethod->id]) }}</li>
            </ol>
        </div>
    </x-slot>
                    ...
                    <div class="row mb-2">
                        <div class="col-lg-8">
                            <div class="mb-3">
                                ...
                                <input type="text" name="name" value="{{ $paymentMethod->name }}" class="form-control" disabled>
                            </div>
                        </div>
                    </div>
                    <div class="border rounded mb-3 p-4">
                        ...
                        <div id="attributes">
                            @foreach($paymentMethod?->attributes ?? [] as $k => $v)
                                <x-payment-method-attribute :key="$k" :value="$v" :removable="false" :readonly="true"/>
                            @endforeach
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="mb-3">
                                <label class="form-check form-switch">
                                    <input name="is_active" value="1" class="form-check-input" type="checkbox" {{ $paymentMethod->is_active ? ' checked' : '' }} disabled>
